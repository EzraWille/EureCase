  ...
  implements OnInit, OnDestroy
{
  ...
  onCountrySelected(country: any) {
    if (country) {
      ...
      this.countryName.setValue(this.selectedCountry.name);
      let foundCountry = this.countryFlag.find(
        (c) => c.name === this.selectedCountry.name
      );
      if (foundCountry) {
        this.countryCode = foundCountry.dial_code;
      } else {
  ...
  // addCertificate() {
  //   this.certificateGroup = this._formBuilder.group({
  //     file: [null, Validators.required],
  //     expirayDate: [null, Validators.required],
  //     fileUploaded: [false],
  //     uploadedFileName: [''],
  //     certificateName: [null, Validators.required],
  //   });
  //   this.certificate.push(this.certificateGroup);
  //   console.log('certificatesValue', this.certificate.value);
  //   console.log('certificates', this.certificate);
  // }
  // removeCertificate(index: number) {
  //   this.certificate.removeAt(index);
  //   this.certificateFilesArray.splice(index, 1);
  //   console.log('certificateFilesArrayAfterremove', this.certificateFilesArray);
  // }

  // handleFileUpload(event: any, index: number) {
  //   const file = event.target.files[0];
  //   if (file) {
  //     const certificateGroup = this.certificate.at(index);
  //     certificateGroup.get('fileUploaded')?.setValue(true);
  //     certificateGroup.get('uploadedFileName')?.setValue(file.name);
  //     console.log('certificatsArray', this.certificate);
  //     this.certificateFilesArray.push(file);
  //     console.log('certificateFilesArrayAdded', this.certificateFilesArray);
  //     // Perform any additional logic you want to do with the uploaded file.
  //   } else {
  //     const certificateGroup = this.certificate.at(index);
  //     certificateGroup.get('fileUploaded')?.setValue(false);
  //     certificateGroup.get('uploadedFileName')?.setValue('');
  //   }
  // }

  // get certificate() {
  //   return this.forthFormGroup.get('certificate') as FormArray;
  // }
  get wholeNumber() {
    return this.firstFormGroup.get('wholeNumber') as FormControl;
  }
  // get expirayDate() {
  //   return this.certificate.get('expirayDate') as FormControl;
  // }
  // get certificateName() {
  //   return this.certificate.get('certificateName') as FormControl;
  // }
  get instituteName() {
  ...
  sumbitForm() {
    ...
    this.certificateFilesArray.forEach((file) => {
      formData.append('certificates', file);
    });
    formData.append('awards', this.profilePicture);
    formData.append('certificatesInfo', JSON.stringify(certificatesInfo));

    this._auth.registerParticipant(formData).subscribe((res) => {
      if (res.status.code === '0') {
        this._alert
          .success('', 'Registration Successful, click OK to verify.')
          .then(() => {
